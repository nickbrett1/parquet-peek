...
# "parquet-peek-dev" (the repo name plus the "-dev"
...
# than adding a second one - a stale "parquet-peek-dev" is overwritten by
...
PROJECT_NAME="parquet-peek"
AGENT_NAME="parquet-peek-dev"
WORKSPACE="/workspaces/parquet-peek"
...
if [ -x "/workspaces/parquet-peek/scripts/agent-dev.sh" ]; then
  "/workspaces/parquet-peek/scripts/agent-dev.sh" start >/dev/null 2>&1 || true
fi
POST_START_HOOK
}
