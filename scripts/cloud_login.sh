...
if command -v doppler &> /dev/null; then
  if doppler whoami &> /dev/null 2>&1; then
    ...
    if doppler login --no-check-version --yes; then
      ...
      if doppler setup --no-interactive --project parquet-peek --config dev; then
        echo "✅ Doppler project parquet-peek/dev configured."
      else
        echo "WARN: doppler setup failed for parquet-peek/dev - the project may not"
        echo "      exist yet. Create it at https://dashboard.doppler.com, then run:"
        echo "      doppler setup --no-interactive --project parquet-peek --config dev"
      fi
    else
