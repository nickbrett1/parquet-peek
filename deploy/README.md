# Deployment runbook: parquet-peek
...
`ghcr.io/nickbrett1/parquet-peek` on every push to
...
2. In `docker-compose.yml`, change the image to the pinned SHA:
   `image: ghcr.io/nickbrett1/parquet-peek:<commit-sha>`
...
## 9. Phone access & HTTPS (Tailscale Serve)

For full PWA standalone mode and Service Worker support on iOS/Android over Tailscale:

1. Run `tailscale serve` on the NAS host pointing to the loopback-bound port:
   ```bash
   tailscale serve --bg http://127.0.0.1:3002
   ```
2. Access `https://parquet-peek.<your-tailnet>.ts.net` from Safari on iOS or Chrome on Android.
3. Tap **Add to Home Screen** for a native standalone dark PWA experience.
